...
class ExportingHistory(IndicatorSpec):
    def __init__(self, ):
        ...
        parent_folder = Path(os.path.expanduser(preferences['todo-file'])).parent.as_posix()
    ...
    def clear_history(self, widget):
        ...
        result = Alert.show_alert('Are you sure?', 'You will remove all history', True)
        ...
        if history_items:
            ...
            with f:
                ...
                for history_item in history_items:
                    started = self.timestam2datetime(history_item.tags['started'])
                    ended = self.timestam2datetime(history_item.tags['ended'])

                    #Localized time
                    step = (self.timestam2datetime(history_item.tags['step_time']) - self.timestam2datetime(0)).total_seconds()
                    total = (self.timestam2datetime(history_item.tags['total_time']) - self.timestam2datetime(0)).total_seconds()
                    if previous_todo != history_item.text:
                        accumulated_time = 0
                        previous_todo = history_item.text
                    accumulated_time += step
                    writer.writerow({
                            'context': re.sub('_', ' ', history_item.contexts[0]) if len(history_item.contexts) else '',
                            'project': re.sub('_', ' ', history_item.projects[0]) if len(history_item.projects) else '',
                            'todo': history_item.text,
                            'description': re.sub('_', ' ', history_item.tags.get('description', '')),
                            ...
                            'step_time': str(datetime.timedelta(seconds=round(step, 0))),
                            'accumulated_time': str(datetime.timedelta(seconds=round(accumulated_time, 0))),
                            'total_time_after_step': str(datetime.timedelta(seconds=round(total, 0)))
                    })
